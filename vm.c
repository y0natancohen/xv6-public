...
void swap_pages(uint page_fault_addr, pde_t *pgdir){
  ...
  // cprintf("swap_pages pick_page_to_replace: %d\n", ind_page_to_replace);
...
void handle_page_fault(uint pgFaultAddr){
  ...
  if (((uint)(*pte) & PTE_PG)) {
    // bring from swap
    cprintf("page fault: starting swap..!\n");
